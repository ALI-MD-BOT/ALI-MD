    ...
    try {
        ...
        try {
            ...
            if (!adminStatus.isBotAdmin) {
                await sock.sendMessage(chatId, {
                    text: '🔒 *ALI-MD* needs admin powers to swing the demotion hammer! 🔨'
                });
                return;
            }
            ...
            await sock.sendMessage(chatId, {
                text: '⚠️ ALI-MD can’t check admin status. Make sure the bot has permissions.'
            });
            return;
        }
        ...
        if (error.data === 429) {
            ...
            try {
                await sock.sendMessage(chatId, {
                    text: '⚠️ Rate limit reached! Let ALI-MD catch its breath. 😮‍💨'
                });
            } catch (retryError) {
            ...
            try {
                await sock.sendMessage(chatId, {
                    text: '❌ ALI-MD failed to demote! Is the user already powerless or bot not admin?'
                });
            } catch (sendError) {
...
async function handleDemotionEvent(sock, groupId, participants, author) {
    try {
        ...
        const demotionMessage = `💢 *AUTO-DEMOTION DETECTED BY ALI-MD* 💢\n\n` +
